...
function getLakeOfFHIR(inDir: string): LakeOfFHIR {
  ...
  const files = getFilesRecursive(inDir).filter(
    file =>
      file.endsWith('.json') &&
...
// If expansions.json is in an output director, it was likely generated by the IG Publisher
// Since it is a generated file and it can be large, we skip processing it
const IGNORED_RESOURCE_LIKE_FILES = [`output${path.sep}expansions.json`];
